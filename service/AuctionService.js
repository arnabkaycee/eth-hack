                        ...
                        if (result && typeof result === 'object' && Object.keys(result).length === 3) {
                            ...
                            for (let i = 0; i < result[0].length; i++) {
                                let bidObj = {
                                    ...
                                    bidAmount: result[1][i],
                                    wattSaving: result[2][i]
                                };
...
exports.createBid = async function (body) {
    ...
    let bidAmountNoteEncrypted = crypto.stringToBytes32(await crypto.encryptMessage(govPublicKey, bidAmountNote.getView()));


    let litreSavingNote = await aztec.crypto.create(govPublicKey, litreSaving);
    let litreSavingNoteEncrypted = crypto.stringToBytes32(await crypto.encryptMessage(govPublicKey, bidAmountNote.getView()));
    ...
    eaContract.methods.placeBid(auctionId, bidId, bidAmountNoteEncrypted, offStartTimestamp,
        offEndTimestamp, litreSavingNoteEncrypted).on('transactionHash')


    const createBidCall = await eaContract.methods.placeBid(auctionId, bidId, bidAmountNoteEncrypted, offStartTimestamp,
        offEndTimestamp, litreSavingNoteEncrypted).send({
        from: addresses,
        gasPrice: "200",
        gas: "999999"
    });
